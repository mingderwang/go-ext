...
var Subcommands = &subcommandExt{
	newExtensionPoint(new(Subcommand)),
}

type subcommandExt struct {
	*extensionPoint
}

func (ep *subcommandExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *subcommandExt) Register(extension Subcommand, name string) bool {
	return ep.register(extension, name)
}

func (ep *subcommandExt) Lookup(name string) Subcommand {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
	return ext.(Subcommand)
}

func (ep *subcommandExt) Select(names []string) []Subcommand {
	var selected []Subcommand
...
func (ep *subcommandExt) All() map[string]Subcommand {
	all := make(map[string]Subcommand)
...
func (ep *subcommandExt) Names() []string {
	var names []string
	for k := range ep.all() {
